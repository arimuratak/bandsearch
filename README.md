...
5. How to use
   ...
   3) Put files of ebsd.py & file.py in same folder as EBSD image file. file.py (put EBSD image file path)　file = ???????/filename.png (or jog, tif) ebsd.py (put information of EBSD image) PC0 = [PCx, PCy, PCz]: origin: upper left corner of the EBSD image
Circle = True or False (True: circle shape, False: rectangle shape)
